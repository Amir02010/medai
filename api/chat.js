      ...
      return {
        ...
        body: {
          ...
          /* Поиск в интернете перед ответом: модель сама решает, когда он
             ...
             Отключается переменной окружения AI_SEARCH=off — например,
             если поиск съедает квоту или ответы становятся слишком долгими. */
          ...(String(process.env.AI_SEARCH || "").toLowerCase() === "off"
            ? {}
            : { tools: [{ googleSearch: {} }] }),
          generationConfig: {
            temperature: 0.3,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    /* 429 при включённом поиске обычно означает, что квота именно на поиск
       исчерпана (у него свой счётчик). Ответить по памяти лучше, чем не
       ответить вовсе. */
    const unsupported =
      upstream.status === 400 && (payload.tools || payload?.generationConfig?.thinkingConfig);
    const searchQuota = upstream.status === 429 && payload.tools;

    if (unsupported || searchQuota) {
